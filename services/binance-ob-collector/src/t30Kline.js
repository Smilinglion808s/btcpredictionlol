...
const COLLECTOR_VERSION = "t30-kline-collector-r1";
const TF_MS = 15 * 60 * 1000;
const LAST_OFFSET_S = 29;
const EXPECTED = 30;
...
export function createT30Collector({
  ingestUrl,
  secret,
  boundaryUrl = null,
  ...
  if (!ingestUrl || !secret) {
    log.warn?.("[t30] disabled: T30_INGEST_URL / T30_INGEST_SECRET not configured");
    return { start() {}, stop() {} };
  }
    ...
    try {
      const res = await fetch(ingestUrl, {
        ...
        headers: {
          ...
          "x-t30-timestamp": timestamp,
          "x-t30-signature": signature,
        },
        body,
      });
      if (!res.ok) {
  ...
  async function triggerBoundary(targetMs) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        log.log?.(`[t30] boundary trigger ${targetTs} -> ${res.status}`);
        if (res.ok) return;
      } catch (error) {
        health.last_boundary_target_ts = targetTs;
