...
test('late U timing and Boise-day budget are route-bound',()=>{
  ...
  const p=routePolicy(base,signal,decision+1000);assert.equal(p.maxEntryAgeMs,485000);assert.equal(p.executionRoute,'maker_only');assert.equal(p.feeReserve,0);
