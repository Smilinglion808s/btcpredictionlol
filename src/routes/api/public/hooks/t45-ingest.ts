// T45 Balanced — signed one-second bar ingest endpoint.
//
...
// pushed here. Offsets 0..44 of each 15-minute UTC candle only; everything else
// is rejected by validateT45Samples before persistence.

import { createFileRoute } from "@tanstack/react-router";
import { createClient } from "@supabase/supabase-js";
import { createHmac, timingSafeEqual } from "crypto";
import { z } from "zod";
import { validateT45Samples } from "@/lib/t45/ingest";
import { T45_STREAM_KEY } from "@/lib/t45/config";
import { auditT45, upsertT45Health, upsertT45Samples } from "@/lib/t45/store.server";
...
export const Route = createFileRoute("/api/public/hooks/t45-ingest")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        const { rows, rejected } = validateT45Samples(body.samples, {
        ...
        let stored = 0;
        try {
          stored = await upsertT45Samples(supabase, rows);
        } catch (e) {
          await auditT45(supabase, "ingest-error", { error: String(e) }, false);
          return Response.json({ ok: false, error: String(e) }, { status: 500 });
        }

        const h = (body.health ?? {}) as Record<string, unknown>;
        const last = rows[rows.length - 1];
        await upsertT45Health(supabase, {
          stream_key: T45_STREAM_KEY,
