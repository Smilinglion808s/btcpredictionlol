...
export const exportUniversalV2 = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const [live, arch, td1Rows, td3Rows, aas96Rows, a96Rows, b4x4Rows, b4x4PolicyShadowRows] = await Promise.all([
    ...
    pageAll<Record<string, unknown>>("b4x4_policy_shadows", "target_candle_ts", 40000),
  ]);
...
async function buildTd1RcExport(variant: string) {
  ...
  return td1Rows.map((r: any) => {
    ...
    return {
      ...
      td1_policy_version: r.td1_policy_version,
      td1_compressed_risk_threshold: r.td1_compressed_risk_threshold,
      td1_compressed_risk_market_condition: r.td1_compressed_risk_market_condition,
      td1_compressed_risk_source_prediction_row_id: r.td1_compressed_risk_source_prediction_row_id,
      td1_compressed_risk_evaluable: r.td1_compressed_risk_evaluable,
      td1_compressed_risk_condition: r.td1_compressed_risk_condition,
      td1_compressed_risk_veto_fired: r.td1_compressed_risk_veto_fired,
      td1_compressed_risk_reason: r.td1_compressed_risk_reason,
      td1_compressed_risk_probability: r.td1_compressed_risk_probability,
      td1_legacy_global_veto_condition: r.td1_legacy_global_veto_condition,
      td1_compressed_risk_counterfactual_direction: r.td1_compressed_risk_counterfactual_direction,
      td1_compressed_risk_counterfactual_result: r.td1_compressed_risk_counterfactual_result,
      td1_compressed_risk_counterfactual_score: r.td1_compressed_risk_counterfactual_score,
...
export const getA96Pending = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const { data } = await sb
    .from("a96_predictions")
    .select("target_candle_ts, final_prediction, selected_layer, base_selected_layer, layer_a_direction, layer_b_direction, decision_reason, fit_selector_override_fired, agreement_veto_fired, distance_from_4_candle_low_bps, mean_2_candle_body_to_range, four_candle_net_displacement, four_candle_total_body_path, four_candle_path_efficiency, efficiency_veto_min, efficiency_veto_max, efficiency_veto_condition, efficiency_veto_fired, variant, mean_2_candle_body_to_range_r4:mean_2_candle_body_to_range, body_ratio_max, body_ratio_condition, body_ratio_veto_fired, four_candle_aligned_wick_pressure, wick_pressure_max, wick_pressure_condition, wick_pressure_veto_fired, prior_macd_hist, prior_atr14, aligned_macd_hist_atr, macd_veto_max, macd_veto_condition, macd_veto_fired, r3_counterfactual_decision, r3_counterfactual_direction, r3_counterfactual_reason, r3_counterfactual_result, target_open, fit_resolved_count_at_prediction, layer_a_net_at_prediction, layer_b_net_at_prediction, resolved_at, feature_history_valid, feature_history_error, base_prediction, actual_direction, actual_open, actual_close, actual_high, actual_low, layer_a_result_score, layer_b_result_score, base_result_score, resolution_attempt_count, last_resolution_error")
