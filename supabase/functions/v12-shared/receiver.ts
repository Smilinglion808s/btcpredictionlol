// Shadow receiver only. No exchange client, order path or forwarding exists.
...
export function createReceiver(route:Route,get:(key:string)=>string|undefined,transport:typeof fetch=fetch,clock:()=>number=Date.now) {
  return async(req:Request):Promise<Response>=>{
    ...
    try{payload=JSON.parse(raw);validateSignal(payload,route,clock());}
    ...
    try{
      const result=await transport(base+'/rest/v1/rpc/record_v12_shadow_signal',{
        method:'POST',headers:{apikey:key,Authorization:'Bearer '+key,'content-type':'application/json'},
        body:JSON.stringify({p_payload:payload,p_hash:hash}),signal:AbortSignal.timeout(5000)});
      if(!result.ok)return reply(503,{error:'SHADOW_RECORD_FAILED'});
      return reply(200,{...await result.json(),mode:'shadow',execution_enabled:false});
