...
BEGIN
  ...
  IF v_side <> 0 AND v_leg = 'T45R2' THEN
    ...
    v_blocked := (v_t IS NULL)
      ...
      -- Live and research observations only ever pair with a V1 record from
      -- the same mode; a mode mismatch or missing mode blocks.
      OR v_t.run_mode IS DISTINCT FROM (p_decision->>'run_mode')
