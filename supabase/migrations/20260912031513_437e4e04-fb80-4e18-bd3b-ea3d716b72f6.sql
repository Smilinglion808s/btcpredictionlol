ALTER TABLE public.v11_scores
  ALTER COLUMN event_cutoff_offset_ms TYPE bigint,
  ALTER COLUMN inputs_persisted_offset_ms TYPE bigint,
  ALTER COLUMN decision_offset_ms TYPE bigint;
ALTER TABLE public.v11_decisions
  ALTER COLUMN event_cutoff_offset_ms TYPE bigint,
  ALTER COLUMN inputs_persisted_offset_ms TYPE bigint,
  ALTER COLUMN decision_offset_ms TYPE bigint;

CREATE OR REPLACE FUNCTION public.v11_commit_observation(p_target_ts timestamp with time zone, p_score jsonb, p_decision jsonb, p_expected_prev_ts timestamp with time zone DEFAULT NULL::timestamp with time zone, p_expected_state_version bigint DEFAULT NULL::bigint, p_allow_backfill boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF NOT p_allow_backfill THEN
    ...
    END IF;
    -- Authoritative predecessor set: shadow context rows UNION the original V1
    -- official opportunity ledger, so an interval never imported into
    -- v11_context_rows cannot be silently skipped.
    WITH owed AS (
      SELECT c.target_ts AS ts
      FROM public.v11_context_rows c
      WHERE c.target_ts > COALESCE(v_prev, '-infinity'::timestamptz)
        AND c.target_ts < p_target_ts
      UNION
      SELECT t.target_open_utc AS ts
      FROM public.c85_targets t
      WHERE t.model_version = 'lite-a-floor4-top10-r1'
        AND t.target_open_utc > COALESCE(v_prev, '-infinity'::timestamptz)
        AND t.target_open_utc < p_target_ts
    ), missing AS (
      SELECT ts FROM owed o
      WHERE NOT EXISTS (SELECT 1 FROM public.v11_decisions d WHERE d.target_ts = o.ts)
    )
    SELECT min(ts), count(*) INTO v_gap, v_gap_count FROM missing;
  ...
  -- Take every row lock that can BLOCK before sampling the publication clock,
  -- so the timestamp reflects the real commit moment and not a moment spent
  -- waiting on the V1 leg lock.
  ...
  -- Truthful publication timing: sampled at the final transaction boundary,
  -- after all blocking locks, with clock_timestamp() (not transaction-start
  -- now()), so a late row cannot be smuggled in as LIVE_SHADOW.
  ...
  -- Cross-leg exclusion re-verified INSIDE the transaction on the locked row.
  IF v_side <> 0 AND v_leg = 'T45R2' THEN
    v_mode_ok := CASE
      WHEN NOT v_have_t OR v_t.run_mode IS NULL THEN false
      WHEN v_mode = 'LIVE_SHADOW' THEN v_t.run_mode = 'LIVE'
      WHEN v_mode IN ('RECOVERY', 'RESEARCH') THEN v_t.run_mode IN ('LIVE', 'RESEARCH')
      ELSE false
    END;

    v_blocked := (NOT v_have_t)
      ...
      OR (v_have_t AND EXISTS (SELECT 1 FROM public.c85_outbox o
  ...
  SELECT p_target_ts,
    ...
    (p_score->>'event_cutoff_offset_ms')::bigint,
    (p_score->>'inputs_persisted_offset_ms')::bigint,
    (p_score->>'decision_offset_ms')::bigint
  ...
  SELECT p_target_ts,
    ...
    v_mode,
    v_evidence,
    (p_decision->>'event_cutoff_offset_ms')::bigint,
    (p_decision->>'inputs_persisted_offset_ms')::bigint,
    (p_decision->>'decision_offset_ms')::bigint,
    v_ceiling,
    v_within
  ON CONFLICT (target_ts) DO NOTHING;
