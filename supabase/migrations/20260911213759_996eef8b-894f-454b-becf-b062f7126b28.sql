ALTER TABLE public.webhook_deliveries
  ...
  ADD COLUMN IF NOT EXISTS attempt_start_offset_ms DOUBLE PRECISION;
...
BEGIN
  ...
  ELSE
    ...
    -- A row built by jsonb_populate_record does NOT pick up column defaults, so
    -- the not-null bookkeeping columns are filled explicitly here.
  ...
  IF p_outbox IS NOT NULL AND p_outbox <> 'null'::jsonb THEN
    v_expires := COALESCE((p_outbox->>'expires_at')::timestamptz, now() + interval '5 seconds');
    ...
    VALUES (p_outbox->>'dedupe_key', v_id, COALESCE(p_outbox->'payload','{}'::jsonb),
            CASE WHEN v_expires <= now() THEN 'EXPIRED' ELSE 'PENDING' END,
            v_expires)
    ON CONFLICT (dedupe_key) DO NOTHING;

    UPDATE public.c85_outbox
       SET state = 'EXPIRED',
           last_error = COALESCE(last_error, 'expired_before_dispatch')
  ...
  -- Version 1 only: return the MINIMAL persisted decision fields from this same
  -- transaction, so the dispatch path does not need a second network read
  -- before it can send. Read back from the table (never from the request body),
  -- so a replayed/altered body cannot change what would be delivered.
  IF v_model = 'lite-a-floor4-top10-r1' THEN
    SELECT jsonb_build_object(
             'id', t.id,
             'model_version', t.model_version,
             'ticker', t.ticker,
             'target_open_utc', t.target_open_utc,
             'run_mode', t.run_mode,
             'status', t.status,
             'final_side', t.final_side,
             'probability_yes', t.probability_yes,
             'admission_rank', t.admission_rank,
             'publication_offset_ms', t.publication_offset_ms,
             'packet_freeze_ns', t.packet_freeze_ns,
             'decision_durable_ns', t.decision_durable_ns,
             'features', t.features
           )
      INTO v_decision
      FROM public.c85_targets t
     WHERE t.id = v_id AND t.model_version = v_model;
  END IF;
